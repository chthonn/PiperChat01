...
describe("Friend Request Flow", () => {
  ...
  test("User should accept friend request", async () => {
    ...
    expect(updatedA.friends.length).toBe(1);

    expect(updatedB.friends.length).toBe(1);

    expect(updatedA.friends[0].id)
      .toBe(String(userB._id));

    expect(updatedB.friends[0].id)
      .toBe(String(userA._id));
